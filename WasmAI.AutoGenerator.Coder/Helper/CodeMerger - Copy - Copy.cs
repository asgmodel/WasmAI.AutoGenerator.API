...
public class AdvGeminiCodeService
{
    ...
    public AdvGeminiCodeService(string apiKey /* Consider secure configuration */ = "AIzaSyALy9VxGquNEppI_YtSjPbKK06JyKALnT4") // Changed default key
    {
...
- For EACH PUBLIC PROPERTY in the Model Structure, create a corresponding private async method to perform validation.
    ...
    - delete async when the function does not contain await
    -If the function has async, the reference must be either ConditionResult.ToFailure or ToSuccess.
    -  dont remove any usespaces in code
    -- If you use the res.Success condition and it is of type bool? if the condition must be as follows (res.Success==true)
...
Only return the C# code, no comments, no explanation, and no extra symbols.";
    ...
    public async Task<string> GenerateeControlCodeFromTextAsync(string description, string infvalid, string vm, string sers, string controlTemplate)
    {
        var instruction = @"You are a C# code generation expert.
Your task is to write C# code based on the user's text description.
Only return the C# code, no comments, no explanation, and no extra symbols.";
        ...
        requestContents.Add(new Content()
        {
            ...
            {
                new Part() { Text = instruction },
                new Part() { Text = description+TmController.GeneratecontrolTemplate(infvalid,vm,sers,controlTemplate) }
            }
        });
